...
import { Trophy, Swords, Shield, Unlock, Lock, ChevronDown, ChevronUp } from 'lucide-react';
  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
      ...
      <div className="flex gap-3 text-[10px] text-muted-foreground mb-1">
        ...
        {best && <span className="text-primary font-bold">Best: {best.total_attempts} tries</span>}
      </div>
      {lockAttempts.slice(0, 5).map((a: any) => (
        <div key={a.id} className="flex items-center gap-2 text-[11px]">
          ...
          {a.is_solved ? (
            ...
            <span className="text-muted-foreground">{a.phase} phase · {a.total_attempts}</span>
          )}
        </div>
      ))}
      {!lock.is_cracked && (
...
export function LockboxLeaderboard({ computed, formalScores, locks, attempts, weekLabel, weekId }: Props) {
  ...
  // Use formal scores if available, otherwise computed
  ...
  if (!hasData) {
    return (
      <div className="glass-card p-8 text-center">
        ...
        <p className="text-[11px] text-muted-foreground">Create your lock and start cracking to earn points</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        <div className="text-[10px] font-bold text-muted-foreground/60 tracking-wider">FULL STANDINGS</div>
      ...
      {locks.length > 0 && (
        <div>
          <button onClick={() => setShowLockResults(!showLockResults)}
            ...
            LOCK RESULTS {showLockResults ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
          </button>
          {showLockResults && (
            <div className="space-y-1.5">
              {locks.map((lock: any) => (
                <div key={lock.id}>
                  <button onClick={() => setExpandedLock(expandedLock === lock.id ? null : lock.id)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-muted-foreground">{lock.is_cracked ? 'Cracked' : 'Defended'}</div>
                    </div>
