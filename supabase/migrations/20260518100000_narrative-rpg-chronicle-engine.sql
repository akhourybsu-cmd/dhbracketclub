...
--
-- Full schema for the Narrative RPG plugin: campaigns + approval flow,
-- characters using the Chronicle Ruleset (Grit/Charm/Cunning/Chaos/Focus),
-- chapters/scenes, story chat messages, dice rolls (denormalized into
-- message metadata for the simple path + a dedicated table for queryable
-- analytics), NPCs/locations/items/clues/factions/clocks, campaign
-- memory + summaries, AI suggestion review queue, approval audit log,
-- and GM-only notes.
--
-- RLS philosophy:
--   • Campaign visibility is computed by helper PG function so policies
--     stay declarative and consistent across all child tables.
--   • GM-only data (gm_notes columns, ai_suggestions, hidden clocks/clues,
--     gm_notes table) is gated by a separate `narrative_is_gm()` helper.
--   • Players can only edit their own character row.
--   • Spectators can read public messages but cannot insert.
-- ═══════════════════════════════════════════════════════════════════
...
-- Single memory row per campaign — append-only history tracked in jsonb.
...
-- ── 10. Dice rolls (queryable analytics layer on top of messages) ──
...
-- Is the user a member of the given campaign (with active status)?
...
-- Is the user the Game Master of the campaign? (Either via the gm_id field
-- on the campaign or via campaign_members role='game_master'.)
...
-- Is the user a club admin (or app admin) for the campaign's club?
...
-- Can the user see this campaign at all? Combines visibility + member +
-- creator + GM + admin rules so policies stay clean. Pending/rejected
-- campaigns are only visible to creator + proposed GM + club admins.
...
as $$
  ...
  select case
    ...
    when (select visibility from c) = 'invite_only' then public.narrative_is_member(_campaign, _user)
...
-- Characters — owners edit their own; GM/admin can manage all.
...
-- Chapters / Scenes — GM/admin write, members read
...
-- Scenes — public_notes visible to all; gm_notes visible only to GM via a
-- view in the client. The row itself is visible to members; client must
-- not render gm_notes for non-GM. (Defense-in-depth: GM-only data lives
-- in separate tables for sensitive content.)
...
-- Messages — visibility-aware. Public messages visible to all who can
-- see the campaign; gm_only visible to GM + club admins; private visible
-- to sender + GM + admins.
...
create policy narrative_messages_insert on public.narrative_messages for insert
  with check (
    sender_id = auth.uid()
    and public.narrative_is_member(campaign_id, auth.uid())
    -- Spectators cannot post gameplay messages
    and (
...
-- NPCs / Locations / Items / Clues / Factions — visibility-aware read,
-- GM/admin write. Hidden rows (visibility='gm_only') only visible to GM.
do $$
declare
  t text;
...
begin
  foreach t in array tables loop
    execute format('drop policy if exists %I_select on public.%I', t || '_visibility', t);
    execute format($f$
...
-- Memory — read for members, gm_only_notes column is part of the row but
-- the client must hide it for non-GMs. Write reserved to GM/admin.
...
-- Summaries — visibility-aware read, GM/admin write
...
-- AI suggestions — GM-only at every step
...
-- Approval events — visible to creator/GM/admin
...
-- GM notes — strictly GM + club admin
